...
/**
 * Class WP_Exchange_Provider_Openexchangerates
 */
class WP_Exchange_Provider_Openexchangerates implements WP_Exchange_Provider {
	 ...
	 *
	 * Note: Free tier always returns USD as base. Cross-rates are computed internally.
	 *
	...
	public function get_name(): string {
		return __( 'Open Exchange Rates', 'wp-exchange' );
	}
	...
	public function get_settings_fields(): array {
		return array(
			array(
				'id'          => 'app_id',
				'label'       => __( 'App ID', 'wp-exchange' ),
				'type'        => 'password',
				'description' => __( 'Enter your Open Exchange Rates App ID.', 'wp-exchange' ),
			),
		);
	}
	...
	public function validate_credentials( array $settings ) {
		...
		if ( empty( $app_id ) ) {
			return new WP_Error( 'missing_app_id', __( 'App ID is required.', 'wp-exchange' ) );
		}
		...
		if ( ! is_array( $data ) || ! isset( $data['rates'] ) ) {
			return new WP_Error( 'invalid_response', __( 'Could not validate App ID.', 'wp-exchange' ) );
		}
